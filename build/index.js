...
import { filterCsvData, getURLs, mapUrlsToCSV, readCSVFile, writeCSVFile } from './helpers.js';
import os from 'os';
...
server.prompt("extract-urls", "Extract URLs from a CSV file based on a given route", {
...
server.tool("read-route-data-from-csv", "Read data from sitemap CSV files and filter rows that matched for the given route", {
...
server.tool("save-route-data-to-csv", "Save filtered route data to a CSV file with URLs organized by subsidiary and language", {
    urls: z.object({
        nl: z.object({
            nl: z.array(z.object({
                language: z.string(),
    ...
    try {
        ...
        const csvData = mapUrlsToCSV(urls);
        ...
        await writeCSVFile(outputFilePath, csvData, ['SUBSIDIARY', 'LANGUAGE', 'URL', 'NODE_ID']);
        return {
            content: [
                {
                    type: "text",
